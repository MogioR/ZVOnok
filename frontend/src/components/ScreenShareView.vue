...
// Auto-switch to single if only 1 sharer
...
// ─── Split mode ────────────────────────────────────────────────────────────
const splitRefs = new Map()  // id → <video>
const splitVols = ref({})
...
function setSplitRef(id, el) {
  if (el) {
    splitRefs.set(id, el)
    // Attach stream immediately when ref is set
...
// Update split videos when streams change
watch(
...
// When switching TO split mode, attach all streams
watch(mode, (m) => {
  if (m === 'split') {
    nextTick(() => {
      props.sharers.forEach((s) => {
...
/* Footer: always visible at bottom of each cell */
.cell-footer {
  position: absolute;
  bottom: 0;
  left: 0;
